...
Variable providers are typically defined in the `.confixrc` or `.confix.repository` files but can also be defined in `.confix.project`.
...
- `confix variables set <name> <value>`: This command sets the value for a specified variable.
