...
public sealed class ConfixValidationSettings
{
    ...
    /// <summary>Explicit application source boundary for startup coverage. Defaults to JSON providers.</summary>
...
/// <summary>At startup check owned JSON paths, without treating ambient environment keys as appsettings.</summary>
internal sealed class CoverageValidator(IServiceProvider services, CoverageSource source) : IValidateOptions<CoverageOptions>
{
    ...
    private static void Collect(IConfigurationProvider provider, string? parent, Dictionary<string, string?> data)
    {
