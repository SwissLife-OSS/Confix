...
public static class ConfigurationValidationPipeline
{
        ...
        bool write)
    {
        ...
        if (settings is null || settings.EffectiveType == ValidationConfiguration.JsonSchema)
        {
            await next(context);
        ...
        {
            ValidationConfiguration.DotnetOptions => new DotnetOptionsValidator(),
            _ => throw new ExitException(
                "Unsupported configuration validation provider: " + settings.EffectiveType)
        };

        var exportSchema = write &&
            ((environment.ExportSchema ?? project.ExportSchema ?? false) ||
        ...
        await new Pipeline(builder =>
            {
                ...
                // Scaffolding needs the composed document, so it follows variable substitution.
                if (write)
                {
                    builder.Use<ScaffoldDotnetDefaultsMiddleware>();
                }

                builder.Use((current, _) =>
                    ValidateAsync(current, validator, settings, write, exportSchema));
            })
        ...
        bool exportSchema)
    {
        ...
        if (write)
        {
            ...
            if (exportSchema && schema is not null)
            {
                ...
                await ConfigurationOutput.ExportSchemaAsync(
                    directory,
                    schema,
