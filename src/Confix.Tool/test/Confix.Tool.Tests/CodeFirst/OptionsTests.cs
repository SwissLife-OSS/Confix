...
public sealed class OptionsTests
{
    ...
    public void RegisteredSectionsExplicitlyAllowsUnownedRoots()
    {
        ...
        ContractValidation.Validate(provider, configuration).Should().Contain(e => e.Contains("Other"));
    }
    ...
    {
        var parent = ValidationConfiguration.Parse(System.Text.Json.Nodes.JsonNode.Parse("""{"type":"dotnet-options","coverage":"strict","framework":"net10.0"}"""))!;
        var child = ValidationConfiguration.Parse(System.Text.Json.Nodes.JsonNode.Parse("""{"coverage":"registeredSections"}"""))!;
        parent.Merge(child).Should().Be(new ValidationConfiguration("dotnet-options", "registeredSections", "net10.0"));
        parent.Merge(new ValidationConfiguration("json-schema")).Should().Be(new ValidationConfiguration("json-schema"));
    ...
    {
        Action parse = () => ValidationConfiguration.Parse(System.Text.Json.Nodes.JsonNode.Parse(json));
    ...
    {
        var parent = ProjectConfiguration.Parse(System.Text.Json.Nodes.JsonNode.Parse("""
            ...
            """));
        var child = ProjectConfiguration.Parse(System.Text.Json.Nodes.JsonNode.Parse("""{"exportSchema":false}"""));
        ...
        var json = System.Text.Json.JsonSerializer.Serialize(merged.Validation);
    ...
    {
        public Absent() => throw new InvalidOperationException("Must not be constructed without configuration.");
    }
    ...
    public void NamedRegistrationsAndRuntimeOverridesUseNormalOptionsPipeline()
    {
        using var configuration = (ConfigurationRoot)new ConfigurationBuilder()
            .AddJsonStream(new MemoryStream(Encoding.UTF8.GetBytes("{\"First\":{\"Host\":\"one\"},\"Second\":{\"Host\":\"two\"}}")))
            .AddInMemoryCollection(new Dictionary<string, string?> { ["Second:Port"] = "70000", ["AMBIENT_KEY"] = "ignored" }).Build();
        var services = new ServiceCollection();
    ...
    public void ValidatorsCanUseExplicitServices()
    {
        ...
        using var provider = services.BuildServiceProvider();
        ContractValidation.Validate(provider, configuration).Should().Contain(e => e.Contains("ServiceChecked:Value"));
    }
    ...
    public sealed class OptionalMail { [Required] public string Host { get; set; } = ""; }
    [ConfixSection("Collection")]
    public sealed class Collection { [ConfixRequiredItems] public List<Mail?> Items { get; set; } = []; }
    public sealed record CheckState(bool Allowed);
    public sealed class CheckAttribute : ValidationAttribute
    {
        protected override ValidationResult? IsValid(object? value, ValidationContext context)
            => ((CheckState)context.GetService(typeof(CheckState))!).Allowed ? ValidationResult.Success : new ValidationResult("Not allowed", [context.MemberName!]);
    }
    [ConfixSection("ServiceChecked")]
    public sealed class ServiceChecked { [Check] public string Value { get; set; } = ""; }

    private static ConfigurationRoot Config(string json) => (ConfigurationRoot)new ConfigurationBuilder()
    ...
    public sealed class Nested
    {
        ...
        [ConfixRequiredKey] public bool Explicit { get; set; }
    }
